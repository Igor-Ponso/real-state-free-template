    ...
    <div class="w-44 shrink-0">
        ...
        <Popover>
            <PopoverTrigger as-child>
                <button aria-label="Filter by bedrooms and bathrooms" :class="[TRIGGER, 'w-full gap-2']">
            ...
            <PopoverContent class="w-72 p-4" align="start">
                ...
                <div v-else class="mt-2 flex gap-2">
                    ...
                    <button v-for="n in BEDROOM_OPTIONS" :key="n" :class="[CHIP, selectedBedrooms.includes(String(n)) ? CHIP_ACTIVE : CHIP_INACTIVE]" @click="selectSingle(selectedBedrooms, String(n))">
                        {{ n }}{{ bedroomsExact ? '' : '+' }}
                    </button>
                </div>
                ...
                <div class="mt-2 flex gap-2">
                    ...
                    <button v-for="n in BATHROOM_OPTIONS" :key="n" :class="[CHIP, selectedBathrooms.includes(n) ? CHIP_ACTIVE : CHIP_INACTIVE]" @click="selectSingle(selectedBathrooms, n)">
                        {{ n }}{{ bathroomsExact ? '' : '+' }}
                    </button>
                </div>
            </PopoverContent>
